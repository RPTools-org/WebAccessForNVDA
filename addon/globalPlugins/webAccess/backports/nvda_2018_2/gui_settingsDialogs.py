...
try:
	...
	from ..nvda_2016_4.gui_nvdaControls import AutoWidthColumnListCtrl
...
class SettingsDialog(wx.Dialog):
	...
	def __new__(cls, *args, **kwargs):
		if next((dlg for dlg in SettingsDialog._instances if isinstance(dlg,cls)),None) or (
			...
			raise gui.SettingsDialog.MultiInstanceError("Only one instance of SettingsDialog can exist at a time")
			pass
...
class MultiCategorySettingsDialog(SettingsDialog):
	...
	def makeSettings(self, settingsSizer):
		...
		self.catListCtrl = AutoWidthColumnListCtrl(
			self,
